...
Задача: На основе этих двух строк **сформировать один конечный промпт** в виде строгого JSON-объекта с полями в порядке: Role, Context, Task, Ramki, Format.  
...
8. Итоговый вывод — **один** JSON, где поле `Ramki` содержит явные операционные инструкции (что сделано и что фабрика встроила), а поле `Format` — операционализированное мышление создаваемого промпта (обязательные секции для исполнителя).
...

{
  ...
  "Ramki": "<Операционные рамки — это МЫШЛЕНИЕ МЕТА-ПРОМПТА (архитектор). Обязательно включи здесь:
...
- Если вход короткий/неинформативный — заполни поля контекста максимально извлечённой информацией и в Ramki укажи, какие уточняющие вопросы нужно задать пользователю, чтобы улучшить промпт.
