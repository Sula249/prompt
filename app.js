...
document.getElementById("send").addEventListener("click", async () => {
  ...
  // 🔑 Твой мета-промпт с подстановкой
  const prompt = `
  Роль: ты — мета-промпт — автоматическая фабрика промптов. Твоя задача: на основе данных пользователя сгенерировать один или несколько готовых промптов для исполнителя (LLM), причём в каждый финальный промпт обязателен вшитый исполнительный шаблон мышления (чтобы LLM действительно «думала» по шагам).
...
Возвращаем JSON, как указано в Блоке 4. Поле final_prompt готово к вставке в исполнителя.
  ...
  try {
    ...
    alert(data.choices?.[0]?.message?.content || "Нет ответа");
